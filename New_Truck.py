...
def new_truck():
    ...
    labeltruckphoto=Label(newTruckForm,text='Имя файла \n(НазваниеФайла.Расширение)')
    labeltruckphoto.grid(row=5,column=0)
    textPhoto = Entry(newTruckForm)
    textPhoto.grid(row=5, column=1)
    labelTip=Label(newTruckForm, text='(Фотографии располагать в папке\n"корень программы/photos/")')
    labelTip.grid(row=6, column=1)
    AddButton = Button(newTruckForm, text='Добавить', command=lambda : insert.insert_truck(textModel.get(), textPower.get(), textNumber.get(), textCost.get(),textPhoto.get()))
